...
def move_to_az_index(move):
    ...
    if move.promotion is not None:
        if move.promotion != chess.QUEEN:
            move_offset = 64 + get_underpromotion_type(move)  # 非皇后升变，range: 64-72
        else:
            move_offset = encode_queen_move(move)  # 升变为皇后或普通皇后移动
...
def az_index_to_move(index, board):
    if index == -1:
    ...
    print(f"Decoding move index {index}: from_file={from_file}, from_rank={from_rank}, move_type={move_type}")

    if move_type >= 64:
        underpromotion_type = inflate_1d_coords_2d(move_type - 64, UNDERPROMOTIONS_SHAPE)
        file_modifier = underpromotion_type[0] - 1  # shift from range 0,..2 to -1,..1
        promotion_piece = underpromotion_type[1] + PROMOTION_PIECE_MODIFIER  # Restore to python-chess piece numbers

        to_file = from_file + file_modifier
        to_rank = 7 if from_rank == 6 else 0  # Under-promotions always move a rank ahead (player-relative)

        print(f"Underpromotion move: to_file={to_file}, to_rank={to_rank}, promotion_piece={promotion_piece}")
    elif move_type >= 56:
        knight_delta = decode_knight_move_delta(move_type - 56)
        to_file = from_file + knight_delta[0]
        to_rank = from_rank + knight_delta[1]

        print(f"Knight move: to_file={to_file}, to_rank={to_rank}")
    else:
        queen_delta = decode_queen_delta(move_type)
        to_file = from_file + queen_delta[0]
        to_rank = from_rank + queen_delta[1]

        print(f"Queen move: to_file={to_file}, to_rank={to_rank}")

        piece = board.piece_at(chess.square(from_file, from_rank))
        if piece and piece.piece_type == chess.PAWN:
            if (piece.color == chess.WHITE and to_rank == 7) or (piece.color == chess.BLACK and to_rank == 0):
                promotion_piece = chess.QUEEN  
                print(f"Pawn promotion detected at ({from_file},{from_rank}) to ({to_file},{to_rank})")
